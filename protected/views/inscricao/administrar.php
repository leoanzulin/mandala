...
function statusDeInscricao($data)
{
    if ($data->status == Inscricao::STATUS_PENDENTE) return "Pré-inscrição realizada";
    else if ($data->status == Inscricao::STATUS_PAGOU_PRE_INSCRICAO) return "Inscrição paga";
    else if ($data->status == Inscricao::STATUS_DOCUMENTOS_VERIFICADOS) return "Documentos validados";
}

?>

<h1>Gerenciar pré-inscrições</h1>
<br>
...
$this->widget('zii.widgets.grid.CGridView', array(
    ...
    'columns' => array(
        ...
        array(
            'header' => 'Documentos',
            'class' => 'CButtonColumn',
            'template' => '{cpf} {rg} {diploma} {comprovante_residencia} {curriculo}',
            'htmlOptions' => array(
                'style' => 'width: 120px',
            ),
            'buttons' => array(
        ...
        array(
            'headerTemplate' => 'Pagou a inscrição?',
            'name' => 'pagou_inscricao',
            'id' => 'pagou_inscricao',
            'value' => '$data->status == 0 ? $data->cpf : ""',
